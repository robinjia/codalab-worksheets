...
def test(ctx):
    ...
    uuid = _run_command([cl, 'run', 'echo hello', '--request-memory', '200m'])
    wait(uuid)
    check_equals('hello', _run_command([cl, 'cat', uuid + '/stdout']))
    uuid1 = _run_command([cl, 'run', 'echo hello2', '--request-memory', '200m'])
    wait(uuid1)
    check_equals('hello2', _run_command([cl, 'cat', uuid1 + '/stdout']))
    # memo tests
    uuid_memo = _run_command([cl, 'run', 'echo hello', '--memoize', '--request-memory', '200m'])
    check_equals(uuid_memo, uuid)

    # Case 2: single dependency
    # target_spec: ":<uuid>"
    uuid_dep = _run_command([cl, 'run', ':{}'.format(uuid), 'echo hello', '--request-memory', '200m'])
    wait(uuid_dep)
    check_equals('hello', _run_command([cl, 'cat', uuid_dep + '/stdout']))
    # memo tests
    uuid_dep_memo = _run_command([cl, 'run', ':{}'.format(uuid), 'echo hello', '--memoize', '--request-memory', '200m'])
    check_equals(uuid_dep_memo, uuid_dep)

    # Case 3: multiple dependencies without key
    # target_spec: ":<uuid_1>, :<uuid_2>"
    uuid_deps = _run_command([cl, 'run', ':{}'.format(uuid), ':{}'.format(uuid1), 'echo hello', '--request-memory', '200m'])
    wait(uuid_deps)
    check_equals('hello', _run_command([cl, 'cat', uuid_deps + '/stdout']))
    # memo tests
    uuid_deps_memo = _run_command(
        [cl, 'run', ':{}'.format(uuid), ':{}'.format(uuid), 'echo hello', '--memoize', '--request-memory', '200m']
    )
    ...
    uuid_multi_alias = _run_command(
        [cl, 'run', 'foo:{}'.format(uuid), 'foo1:{}'.format(uuid), 'echo hello', '--request-memory', '200m']
    )
    wait(uuid_multi_alias)
    check_equals('hello', _run_command([cl, 'cat', uuid_multi_alias + '/stdout']))
    # memo tests
    uuid_multi_alias_memo = _run_command(
        [cl, 'run', 'foo:{}'.format(uuid), 'foo1:{}'.format(uuid), 'echo hello', '--memoize', '--request-memory', '200m']
    )
    check_equals(uuid_multi_alias_memo, uuid_multi_alias)

    # Case 5: duplicate dependencies
    # target_spec: ":<uuid>, :<uuid>"
    uuid_dup_deps = _run_command([cl, 'run', ':{}'.format(uuid), ':{}'.format(uuid), 'echo hello', '--request-memory', '200m'])
    wait(uuid_dup_deps)
    ...
    uuid_a_b = _run_command([cl, 'run', 'a:{}'.format(uuid), 'b:{}'.format(uuid1), 'echo a_b', '--request-memory', '200m'])
    wait(uuid_a_b)
    ...
    uuid_a_b_memo = _run_command(
        [cl, 'run', 'a:{}'.format(uuid), 'b:{}'.format(uuid1), 'echo a_b', '--memoize', '--request-memory', '200m']
    )
    check_equals(uuid_a_b_memo, uuid_a_b)
    uuid_b_a_memo = _run_command(
        [cl, 'run', 'b:{}'.format(uuid), 'a:{}'.format(uuid1), 'echo b_a', '--memoize', '--request-memory', '200m']
    )
    ...
    uuid_b_a_order_memo = _run_command(
        [cl, 'run', 'a:{}'.format(uuid1), 'b:{}'.format(uuid), 'echo b_a', '--memoize', '--request-memory', '200m']
    )
